...
import kotlinx.android.synthetic.main.view_video_user_small_item.view.*

/**
 * Created by hang on 2020/6/16.
 * 上麦用户视角 小窗口
 */
class LiveVideoUserSmallItemView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private fun updateView() {
        close_video.isVisible = mUserId == JohnUser.getSharedUser().userID && enableVideo
        close_audio.isVisible = mUserId == JohnUser.getSharedUser().userID && enableVideo
    }

    init {
        layoutInflaterKtToParentAttach(R.layout.view_video_user_small_item)
    ...
    fun showDisableVideoHintPop() {
        if (mUserId == JohnUser.getSharedUser().userID) {
            close_video.showDisableVideoHintPop(dip(191), dip(52), -dip(66), false) {
                mPopupWindow = it
            }
        }
    }
