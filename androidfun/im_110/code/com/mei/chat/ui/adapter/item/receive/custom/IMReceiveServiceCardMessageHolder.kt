...
import kotlinx.android.synthetic.main.im_row_receive_service_card_item.view.*
 ...
 */
class IMReceiveServiceCardMessageHolder(itemView: View) : IMReceiveBaseMessageHolder(itemView) {
    private var mTvPrice: TextView = itemView.findViewById(R.id.im_receive_service_price_tv)
    private var mTvServiceName: TextView = itemView.findViewById(R.id.im_receive_service_name_tv)
    private var mTvHint: TextView = itemView.findViewById(R.id.im_receive_specification_tv)
    private var mTvCount: TextView = itemView.findViewById(R.id.im_receive_service_count_tv)
    ...
    override fun refreshChild(msg: Message) {
        itemView.apply {
            ...
            if (data?.serviceInfo == null) {
                im_receive_service_unit.isVisible = false
                unKnownMessageLoad()
            } else {
                data.serviceInfo?.let {
                    when (it.cardType) {
                        ...
                        ServiceInfo.CardType.TYPE_RECHARGE -> {
                            im_receive_service_unit.isVisible = true
