 ...
 */
class IMReceiveCourseCardMessageHolder(itemView: View) : IMReceiveBaseMessageHolder(itemView) {
        ...
        itemView.apply {
            ...
            when (data?.courseInfo?.cardType) {
                CourseInfo.CardType.TYPE_NORMAL
                    ...
                    im_receive_course_price_tv.isVisible = true
                    im_receive_course_unit.isVisible = true
                    im_receive_original_price.isVisible = data.courseInfo?.crossedPrice != 0
                    ...
                    if (originalPrice.isNotEmpty()) {
                        ...
                        im_receive_original_price.text = ss
                    }
                    im_receive_course_name_tv.text = data.courseInfo?.courseName.orEmpty().decodeUrl()
                    im_receive_course_price_tv.text = "${data.courseInfo?.cost}"
                    im_receive_course_tv.text = hint
                    im_receive_course_tv.updateLayoutParams<ConstraintLayout.LayoutParams> {
                        leftMargin = dip(5)
                    }
                    ...
                    im_receive_course_unit.isVisible = false
