 ...
 */
class IMReceiveVoiceMessageHolder(itemView: View) : IMReceiveBaseMessageHolder(itemView) {
    ...
    override fun refreshChild(msg: Message) {
        (msg as? VoiceMessage)?.let {
            ...
            itemView.findViewById<ImageView>(R.id.im_message_voice_img)?.let {
                it.setImageResource(if (bindingAdapterPosition == playVoicePosition) R.drawable.chat_voice_from_icon else R.drawable.chat_chatfrom_voice_playing_f3)
                (it.drawable as? AnimationDrawable)?.start()
            }
            itemView.findViewById<View>(R.id.im_voice_read_state)?.isVisible = !it.isListened()
