...
import com.mei.video.ui.putVideo
import com.mei.video.ui.selectedVideo
import java.io.File
 ...
 */
internal class VideoPickerAdapter(val list: ArrayList<VideoMediaEntity?>, val itemClick: (Int, VideoMediaEntity?) -> Unit) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        val selected = selectedVideo.any { it.id == item?.id }
        ...
        holder.itemView.findViewById<ImageView>(R.id.select_state_img)?.apply {
            ...
            setOnClickListener {
                if (selectedVideo.any { it.id == item?.id }) {
                    selectedVideo.removeAll { it.id == item?.id }
                    this@VideoPickerAdapter.notifyItemChanged(position)
                } else if (item != null && putVideo(item)) {
                    this@VideoPickerAdapter.notifyItemChanged(position)
                } else if (selectedVideo.size <= 9 && item != null && putVideo(item)) {
                    this@VideoPickerAdapter.notifyItemChanged(position)
